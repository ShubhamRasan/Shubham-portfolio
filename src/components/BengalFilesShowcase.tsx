...
export interface BengalSlideItem {
  id: string;
  title: string;
  subtitle: string;
  image: string;
...
export const bengalSlides: BengalSlideItem[] = [
  {
    id: 'bengal-1',
    title: 'The Bengal Files: Master Theatrical Key Art',
    subtitle: 'Mithun Chakraborty • A Vivek Ranjan Agnihotri Film',
    image: '/Portfolio/Zee5%20Statics/Bengal%20Files%20POSTERS/BGL-FILES_.jpg',
    resolution: '1080 × 1440 PX • 300 DPI',
    coreTechnique: 'Chiaroscuro Silhouette Lighting & Historical Gravitas',
    explanation: 'Shubham’s most acclaimed and celebrated cinematic poster. Built with dramatic chiaroscuro contrast illuminating Mithun Chakraborty’s weathered profile against fiery crimson embers, textured archival paper backdrops, and bespoke Bengali script-inspired theatrical typography.',
    tools: ['Adobe Photoshop', 'Chiaroscuro Silhouette Grading', 'Bengali Typographic Accent', 'Atmospheric Embers'],
    colors: ['#991B1B', '#D97706', '#171717', '#FEF08A']
  },
  {
    id: 'bengal-2',
    title: 'The Delhi Files: Bengal Chapter Teaser Static',
    subtitle: 'Pallavi Joshi & Anupam Kher • Historical Document Composite',
    image: '/Portfolio/Zee5%20Statics/Bengal%20Files%20POSTERS/Bengal-File-Static-2-Recovered.jpg',
    resolution: '1080 × 1440 PX • 300 DPI',
    coreTechnique: 'Dual Character Balance & Archival Parchment Texture',
    explanation: 'Constructed an emotionally grounded teaser poster balancing Pallavi Joshi and Anupam Kher. Utilized distressed vintage government document layers, sepia-amber tonal gradation, and subtle film grain to evoke real historical gravity and suppressed truth.',
    tools: ['Adobe Photoshop', 'Archival Paper Displacement', 'Dual-Tone Sepia Grading', 'Historical Asset Stacking'],
    colors: ['#78350F', '#D97706', '#1C1917', '#E7E5E4']
  },
  {
    id: 'bengal-3',
    title: 'Character Mystery Teaser: Crimson Rim Lighting',
    subtitle: 'Theatrical Release Teaser • High-Contrast Silhouette Option',
    image: '/Portfolio/Zee5%20Statics/Bengal%20Files%20POSTERS/Bangal-poster-OPT-1.jpg',
    resolution: '1080 × 1440 PX • 300 DPI',
    coreTechnique: 'Violent Crimson Edge Lighting & Monochromatic Contrast',
    explanation: 'A stark, suspense-heavy teaser option featuring an intense low-angle profile bathed in intense scarlet backlighting. Designed to create maximum theatrical impact on hoarding billboards and streaming home screens.',
    tools: ['Adobe Photoshop', 'Specular Rim Highlights', 'High-Pass Frequency Separation', 'Deep Black Level Balancing'],
    colors: ['#DC2626', '#7F1D1D', '#0A0A0A', '#FFFFFF']
  }
];

export default function BengalFilesShowcase() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isLightboxOpen, setIsLightboxOpen] = useState(false);
  const [zoomLevel, setZoomLevel] = useState(1);
  const [isPaused, setIsPaused] = useState(false);

  const currentSlide = bengalSlides[currentIndex] || bengalSlides[0];

  const handleNext = () => {
    setCurrentIndex((prev) => (prev < bengalSlides.length - 1 ? prev + 1 : 0));
  };

  const handlePrev = () => {
    setCurrentIndex((prev) => (prev > 0 ? prev - 1 : bengalSlides.length - 1));
  };

  // Auto-slide effect (advances every 4 seconds, pauses on hover or lightbox)
  useEffect(() => {
    if (isLightboxOpen || isPaused || bengalSlides.length <= 1) return;
  ...
  }, [currentIndex, isLightboxOpen, isPaused, bengalSlides.length]);
      ...
      <div className="flex flex-wrap items-center justify-between border-b border-[#222222] pb-4 mb-6 gap-3 text-left">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-mono text-[9px] uppercase tracking-widest text-amber-400 font-bold flex items-center gap-1.5">
              ...
              MOST APPRECIATED WORK • ZEE STUDIOS & I AM BUDDHA PRODUCTION
            </span>
          </div>
          <h3 className="font-display text-2xl sm:text-3xl font-extrabold uppercase tracking-tight text-white">
            The Bengal Files: Official Key Art & Cinema Posters
          </h3>
          <p className="font-mono text-[10px] text-zinc-400 uppercase tracking-wider mt-0.5">
            A Vivek Ranjan Agnihotri Film • Mithun Chakraborty • Pallavi Joshi • Anupam Kher
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <span className="font-mono text-xs font-bold text-zinc-400">
            {currentIndex + 1} / {bengalSlides.length} POSTERS
          </span>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 lg:items-start">
        ...
        <div className="lg:col-span-6 space-y-5 text-left">
          ...
          <div className="space-y-2">
            <span className="text-[10px] font-mono uppercase bg-[#1f1212] text-rose-300 border border-rose-900/50 px-2.5 py-0.5 rounded-none tracking-widest inline-flex items-center gap-1">
              <Flame size={11} className="text-rose-400" />
              Theatrical Poster Craft
            </span>
          ...
          <div className="pt-2">
            ...
            >
              <Maximize2 size={13} />
              <span>Fullscreen Poster Inspector</span>
            </button>
          </div>
      ...
      {isLightboxOpen && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex flex-col p-4 sm:p-6">
          ...
          <div className="border-t border-[#222] pt-3 text-center flex flex-wrap items-center justify-between text-[10px] font-mono text-zinc-400">
            <span>KEY ART MASTER • ZEE STUDIOS THEATRICAL ARCHIVE</span>
